...
import { enfa as originalEnfa, startStateENFA as originalStartStates, acceptingStatesENFA as originalAcceptingStates } from '../js/no5/enfa.js';

async function fetchNfaValues() {
    try {
        // Gunakan nilai yang diimpor dari nfa.js
        const enfa = originalEnfa;
        const startStateENFA = originalStartStates;
        const acceptingStatesENFA = originalAcceptingStates;

        // Set nilai variabel ke dalam elemen input
        document.getElementById('enfa').value = JSON.stringify(enfa);
        document.getElementById('startStates').value = JSON.stringify(startStateENFA);
        document.getElementById('acceptingStates').value = JSON.stringify(acceptingStatesENFA);
    } catch (error) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('saveButton').addEventListener('click', () => {
        ...
        const enfaInput = JSON.parse(document.getElementById('enfa').value);
