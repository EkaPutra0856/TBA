...
# Generate and save images for NFA
nfa_transitions = nfa.get_nfa()
if nfa_transitions:
    start_state = nfa_transitions.get('_start_state')  # Get start state
    accepting_states = nfa_transitions.get('_accepting_states')  # Get accepting states

    generate_image(nfa_transitions, os.path.join(img_folder, "nfa"), start_state, accepting_states)
